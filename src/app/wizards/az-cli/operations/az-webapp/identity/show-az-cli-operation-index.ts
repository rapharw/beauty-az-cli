...
export default class AzCliAzWebAppIdentityAssignOperationIndex extends OperationIndex {
    ...
    getChoice(): Choice {
        return new Choice(
            `${process.env.AZCLI_AZ_WEBAPP_IDENTITY_SHOW_CHOICE_NAME}`,
            `${process.env.AZCLI_AZ_WEBAPP_IDENTITY_SHOW_CHOICE_SHORT}`,
            `${process.env.AZCLI_AZ_WEBAPP_IDENTITY_SHOW_CHOICE_VALUE}`);
    }
